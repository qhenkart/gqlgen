...
func TestImportPathForDir(t *testing.T) {
	...
	assert.Equal(t, "github.com/qhenkart/gqlgen/internal/code", ImportPathForDir(wd))
	assert.Equal(t, "github.com/qhenkart/gqlgen/api", ImportPathForDir(filepath.Join(wd, "..", "..", "api")))

	// doesnt contain go code, but should still give a valid import path
	assert.Equal(t, "github.com/qhenkart/gqlgen/docs", ImportPathForDir(filepath.Join(wd, "..", "..", "docs")))

	// directory does not exist
	assert.Equal(t, "github.com/qhenkart/gqlgen/dos", ImportPathForDir(filepath.Join(wd, "..", "..", "dos")))
...
func TestNameForPackage(t *testing.T) {
	assert.Equal(t, "api", NameForPackage("github.com/qhenkart/gqlgen/api"))

	// does not contain go code, should still give a valid name
	assert.Equal(t, "docs", NameForPackage("github.com/qhenkart/gqlgen/docs"))
